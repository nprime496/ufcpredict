...
#     'Choose your second fighter?',
#     tuple(liste_athletes.keys()))
# #col2.write('Fighter 2:', athlete2)

# selected_ = [(col1,athlete1),(col2,athlete2)]
# for col,athlete in selected_:
#     #input()
...
#     #fighters.append(img)
#     col.subheader(" ".join([e.capitalize() for e in athlete.split(" ")]))
...
# data,numerical_columns = preprocess(merged_stats)
...
# test_transformed = mylgbm[:-1].transform(data)


# explainer = shap.TreeExplainer(combinedlgbm)#mylgbm.named_steps["trained_model"]) #mylgbm.named_steps["trained_model"] not used yet because we don't want finalized model (aka trained on validation)
# shap_values = explainer.shap_values(test_transformed)
# print(list(test_transformed.columns))
# print(len(test_transformed.columns))
# #print(test_transformed['Reach_cms_diff'])
# # Worst predictions on validation set
# prediction = predict_model(mylgbm,data)[['Score','Label']]#+descr_columns+['R_total_fights','B_total_fights']]
...
# st_shap(shap.plots._waterfall.waterfall_legacy(explainer.expected_value[1], shap_values[1][fight_idx,:],test_transformed.loc[fight_idx,:],max_display=20))
